...
class BuildingConstraints:
    ...
    floors: int = 1
    building_type: str = "house"   # Automatically tracked building categorisation
...
class RuleBasedExtractor:
    ...
    TYPE_PATTERNS = re.compile(
        r"\b(house|apartment|office|skyscraper|villa|mansion|complex|building)\b", re.I
    )
    ...
    def extract(self, text: str) -> BuildingConstraints:
        ...
        # 7. Building Type
        type_match = self.TYPE_PATTERNS.search(text)
        if type_match:
            b_type = type_match.group(1).lower()
            if b_type in ("mansion", "villa"):
                b_type = "house"
            elif b_type in ("complex", "building"):
                b_type = "apartment"
            constraints.building_type = b_type
