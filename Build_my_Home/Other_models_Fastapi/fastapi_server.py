...
# We instantiate bim_app here so models load once. 
# BIMPipeline.run() is purely functional and stateless per request.
bim_app = BIMApp()
...
if FASTAPI_AVAILABLE:
    ...
    async def generate(req: GenerateRequest):
        ...
        if not req.requirement or len(req.requirement.strip()) < 3:
            raise HTTPException(status_code=400, detail="Requirement text is too short or empty.")
            
        try:
            # bim_app.generate() is thread-safe and stateless per request natively over the pipeline
            result = bim_app.generate(req.requirement, req.save_outputs)
            print(f"Generated successfully. Output keys: {list(result.keys())}")
            return result
    ...
    @app.post("/structural")
    async def structural_only(req: StructuralRequest):
        """Generate columns and beams from floor layout."""
        try:
            from structural_grid import MultiFloorStructuralGrid
            struct_model = MultiFloorStructuralGrid()
            struct_model.train(verbose=False)
            structural_all = struct_model.generate_all(req.building, verbose=False)
            return {f"floor_{fn}": g.to_dict() for fn, g in (structural_all or {}).items()}
        except ImportError:
            # Fallback if structural grid fails
            result = {}
            for key in req.building.keys():
                cols = []
                for x in range(0, 20+1, 3):
                    for y in range(0, 20+1, 3):
                        cols.append([float(x), float(y)])
                result[key] = {"columns": cols, "beams": [], "slabs": []}
            return result

    @app.post("/schedule")
    async def schedule_only(req: ScheduleRequest):
        """Generate task list and CPM/PERT schedule."""
        from task_engine import ConstructionTaskEngine
        from scheduler import ProjectScheduler
        eng = ConstructionTaskEngine()
        all_rooms = [r for fl in req.building.values() for r in fl]
        s1_struct = list(req.structural.values())[0] if req.structural else {}
        eng.generate(
            rooms      = all_rooms,
            structural = s1_struct,
            mep        = req.mep,
            n_floors   = req.n_floors,
            area_m2    = req.area_m2,
        )
        sched_system = ProjectScheduler()
        schedule_result = sched_system.compute(eng.tasks, eng.get_dag())
        return {
            "tasks": [t.to_dict() for t in eng.tasks],
            "schedule": schedule_result.to_dict() if schedule_result else {}
        }
...
if __name__ == "__main__":
    if FASTAPI_AVAILABLE and "--serve" in sys.argv:
        import uvicorn
        print("Starting BIM API server on http://localhost:8001")
        uvicorn.run("fastapi_server:app", host="0.0.0.0", port=8001, reload=False)
    else:
        # Standalone demo using sys.argv
        if len(sys.argv) > 1 and sys.argv[1] != "--serve":
            user_input = " ".join(sys.argv[1:])
        else:
            print("Error: Please provide a natural language requirement.")
            print("Example: python fastapi_server.py '3 floor house with 4 bedrooms'")
            sys.exit(1)
            
        out = Path("outputs"); out.mkdir(exist_ok=True)

        bim_app.startup()
        result_dict = bim_app.generate(user_input, save_outputs=True)
        ...
        print("\nPipeline execution complete. See outputs/ directory.")
